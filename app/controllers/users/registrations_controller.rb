...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def create_sending
    ...
    unless @sending.valid?
      flash.now[:alert] = @sending.errors.full_messages
      render :new_sending and return
    end
    @user.build_profile(@profile.attributes)
    @user.build_sending(@sending.attributes)
    @user.save
    sign_in(:user, @user)
  ...
  def profile_params
    params.require(:profile).permit(:first_name, :family_name, :first_name_kana, :family_name_kana, :birth_date)
  end
